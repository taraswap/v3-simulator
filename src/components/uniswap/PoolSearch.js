...
const SearchItemsStatsPerp = (props) => {
  ...
  useEffect(() => {
    if (props.perpStatsData && props.perpStatsData.length) {
      const statTemp = props.perpStatsData.find(
        (f) =>
          f.marketSymbol ===
          `${props.item.token0.symbol}/${props.item.token1.symbol}`
      );
  ...
  return (
    <Fragment>
      <div className={styles["search-item-symbol"]}>
        {props.item.token0.symbol} / {item.token1.symbol}
      </div>
      &nbsp;
      <div className={styles["search-item-base-apr"]}>{`${parsePrice(
        stat.lowerBaseApr,
        true
      )}% - ${parsePrice(stat.upperBaseApr, true)}%`}</div>
      &nbsp;
      <div className={styles["search-item-reward-usd"]}>{`${parsePrice(
        stat.lowerRewardApr,
        true
      )}% - ${parsePrice(stat.upperRewardApr, true)}%`}</div>
      &nbsp;
    </Fragment>
  );
};
...
const SearchItemsStats = (props) => {
  ...
  return (
    <Fragment>
      ...
      &nbsp;
      <div>
        <div className={styles["search-item-fee-tier"]}>{`${
          item.feeTier / 10000
        }%`}</div>
      </div>
      &nbsp;
      <div className={styles["search-item-tvl"]}>{`$${formatLargeNumber(
        item.totalValueLockedUSD
      )}`}</div>
      &nbsp;
      <div className={styles["search-item-volume-usd"]}>{`$${formatLargeNumber(
        item.poolDayData[0].volumeUSD
      )}`}</div>
      &nbsp;
      <div className={styles["search-item-fee-usd"]}>{`$${formatLargeNumber(
        item.poolDayData[0].feesUSD
      )}`}</div>
      &nbsp;
    </Fragment>
  );
...
const SearchItemsStatsContainer = (props) => {
  const list = props.items.map((item) => (
    ...
    >
      {props.baseTokenHidden ? (
        <SearchItemsStatsPerp
          item={item}
          perpStatsData={props.perpStatsData}
        ></SearchItemsStatsPerp>
      ) : (
        <SearchItemsStats item={item}></SearchItemsStats>
      )}
    </li>
  ));
...
const EmptySearchItems = (props) => {
  return (
    <div
      className={styles["search-items-empty"]}
      style={{ display: props.visibility || "flex" }}
    >
      <div className={styles["magic-cat-container"]}>
        <div className={styles["magic-cat"]}>
          <img
            src={cat}
            alt="cat"
            style={{ width: "150px", height: "150px", marginTop: "2px" }}
          ></img>
        </div>
      </div>
      <div>
        Sorry, we can't find any Pools <br></br> for that search.
      </div>
    </div>
  );
};
...
const SearchLabels = (props) => {
  const [labelsDef, setLabelsDef] = useState([
    {
      name: "Pool",
      sortable: false,
      sortClass: "sort-icon-disabled",
      labelClass: "search-label-symbol",
    },
    {
      name: "Fee Tier",
      sortable: false,
      sortClass: "sort-icon-disabled",
      labelClass: "search-label-fee-tier",
    },
    {
      name: "TVL",
      sortable: true,
      sortClass: "sort-icon-show",
      labelClass: "search-label-tvl",
      field: "totalValueLockedUSD",
    },
    {
      name: "Volume 24h",
      sortable: true,
      sortClass: "sort-icon-hide",
      labelClass: "search-label-volume-usd",
      field: "volumeUSD",
    },
    {
      name: "Fees 24h",
      sortable: true,
      sortClass: "sort-icon-hide",
      labelClass: "search-label-fee-usd",
      field: "feesUSD",
    },
  ]);

  const [labelsPerp, setLabelsPerp] = useState([
    {
      name: "Pool",
      sortable: false,
      sortClass: "sort-icon-disabled",
    ...
    },
    {
      ...
      sortable: true,
      sortClass: "sort-icon-show",
      labelClass: "search-label-base-apr",
      field: "lowerBaseApr",
    },
    {
      name: "Reward APR%",
      sortable: true,
      sortClass: "sort-icon-hide",
      labelClass: "search-label-reward-apr",
      field: "lowerRewardApr",
    },
  ]);
  ...
  const handleLabelSort = (label) => {
    ...
    labels.forEach((d) => {
      if (d.sortable) {
        return d.name === label.name
          ? (d.sortClass = "sort-icon-show")
          : (d.sortClass = "sort-icon-hide");
      }
    });
  };
  ...
  const searchLabels = labels.map((d) => {
    return (
      <div
        key={d.name}
        className={`${styles["search-label"]} ${styles[d.labelClass]}`}
        onMouseDown={() => handleLabelSort(d)}
      >
        <span className={styles[d.sortClass]}>⬍&nbsp;</span>
        <span>{d.name}</span>
      </div>
    );
  });
  ...
  return (
    <div
      className={styles["search-labels"]}
      style={{ display: props.visibility }}
    >
      {searchLabels}
    </div>
...
const SearchResults = (props) => {
  ...
  const searchStringIsAnId = (searchString) =>
    searchString.length &&
    searchString.length === 42 &&
  ...
  const searchForPool = async (abortController, searchString, protocol) => {
    if (searchStringIsAnId(searchString))
      return await poolsByTokenId(
        searchString,
        abortController.signal,
        protocol
      );

    const tokenPairs = await tokensBySymbol(
      searchString,
      abortController.signal,
      protocol
    );
    if (tokenPairs && tokenPairs.hasOwnProperty("error")) return null;
    return tokenPairs && tokenPairs.length && tokenPairs.length > 0
      ? await poolsByTokenIds(
          tokenPairs.map((d) => d.id),
          abortController.signal,
          protocolID
        )
      : "empty";
    ...
    if (props.visibility !== "none") {
      if (props.customSearch) {
        ...
        if (searchStringIsValid(props.searchString)) {
          searchForPool(
            searchStringController.current,
            props.searchString,
            protocolID
          ).then((searchResults) => {
  ...
  const handleLabelClick = (label, perp) => {
    ...
    if (perp) {
      ...
      const sortBy = label.field;
      const tempResults = searchResults.sort((a, b) => {
        return parseFloat(a["poolDayData"][0][sortBy]) >
          parseFloat(b["poolDayData"][0][sortBy])
          ? -1
          : 1;
      });
      setSearchResults(tempResults);
  ...
  return (
    <LoadingSearchItems visibility={props.visibility}></LoadingSearchItems>
  );
};
...
const Protocol = (props) => {
  ...
  useEffect(() => {
    const protocols =
      props.protocols && props.protocols.length && props.protocols.length > 0
        ? selectProtocolsByIds(protocolsAll, props.protocols)
        : protocolsAll;
  ...
  const protocols = protocolsSelected.map((prot) => {
    return (
      <button
        ...
        className={
          protocolID === prot.id
            ? styles["protocol-selected"]
            : styles["protocol-not-selected"]
        }
        style={{ display: props.visibility }}
  ...
  return (
    <Fragment>
      ...
      <div
        className={
          visibility === "none"
            ? styles["results-container-hidden"]
      ...
      >
        ...
        >
          <Protocol
            pageStyle={props.pageStyle}
            page={props.page}
            handleProtocolChange={handleDisableBlur}
            visibility={visibility}
