...
const UniswapSimulator = (props) => {
  ...
  const handleResize = () => {
    ...
    docEl.style.setProperty(
      "--window-height",
      parseInt(window.innerHeight) + "px"
    );
  ...
  useEffect(() => {
    ...
    if (protocol === 842) {
      poolById(
        "0x6f6e59d10570ef8491fd9b5fe804c61fbc285ca6",
        ...
        >
          ...
          {protocol === 2 ? (
            <div className={styles["arbitrum-error-message"]}>
              {" "}
              Sorry, we're not able to genarate an accurate backtest for
              Arbitrum currently. We'll be sure to add it once accurate data
