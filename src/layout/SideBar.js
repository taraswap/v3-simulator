  ...
  return (
    ...
    >
      <div className={styles["input-container"]}>
        ...
        <br></br>
        <input
          className={`${props.pageStyle["input"]} ${styles["toggle-input"]}`}
          label="Base Token"
          value={baseToken ? baseToken.symbol : ""}
        ></input>
        <ToggleButton
          pageStyle={props.pageStyle}
          onClick={handlePriceToggle}
          alt="toggle base currency"
        ></ToggleButton>
      </div>
    </div>
  );
...
const Investment = (props) => {
  ...
  return (
    <div className={styles["input-container"]}>
      ...
      >{`Investment ${baseToken.symbol}`}</label>
      <input
        type="number"
        className={`${styles["default-input"]} ${props.pageStyle["input"]}`}
        label="Base Token"
        value={investment}
...
const CurrentPrice = (props) => {
  ...
  const handleRefresh = () => {
    ...
    PoolCurrentPrices(abortController.current.signal, protocol.id, poolID).then(
      (d) => {
  ...
  return (
    <div className={styles["input-container"]}>
      ...
      <RefreshButton
        ...
        alt="Refresh Current Price"
      ></RefreshButton>
      <input
        className={`${props.pageStyle["input"]} ${styles["default-input"]}`}
        label="Current Price"
        value={parsePrice(currentPrice)}
        onChange={(e) => handleInputChange(e)}
      ></input>
    </div>
  );
...
const SideBar = (props) => {
  const baseToken =
    props.baseTokenHidden === true ? (
      false
    ) : (
      <BaseToken pageStyle={props.pageStyle}></BaseToken>
    );

  return (
    <Fragment>
      <div
        className={`${styles["pool-search"]} ${props.pageStyle["outer-glow"]} `}
      >
        <div
          className={
            props.pageStyle["sub-container"]
      ...
      <div
        className={`${styles["sidebar"]} ${props.pageStyle["outer-glow"]} `}
      ...
      >
        <div>
          ...
          <StrategyRange
            page={props.page}
            pageStyle={props.pageStyle}
            leverageHidden={props.leverageHidden}
          ></StrategyRange>
        </div>
      </div>
    </Fragment>
