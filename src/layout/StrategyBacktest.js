...
import {
  ...
  tokensForStrategy,
} from "../helpers/uniswap/liquidity";
import { maxInArray, parsePrice } from "../helpers/numbers";
import {
  calcFees,
  pivotFeeData,
  backtestIndicators,
} from "../helpers/uniswap/backtest";
...
import {
  selectSelectedStrategyRanges,
  selectStrategyRangeType,
...
} from "../components/uniswap/BacktestTotalReturn";
  ...
  return (
    <div className={styles["strategy-toggle-container"]}>
      <ToggleButtonsFlex
        ...
        buttons={buttons}
        className={styles["strategy-toggle"]}
  ...
  return (
    <div className={props.className}>
      <BacktestTotalReturn
        ...
        className={`${styles["strategy-breakdown-chart"]} ${
          styles["strategy-breakdown-chart-left"]
        } ${props.pageStyle ? props.pageStyle["inner-glow"] : "inner-glow"}`}
        classNameTitle={`${styles["strategy-breakdown-total-return-title"]} sub-title`}
        data={props.data}
        strategy={props.strategy}
        totalReturnKeys={props.totalReturnKeys}
      ></BacktestTotalReturn>
      <div
        page={props.page}
        className={`${styles["strategy-breakdown-total-return-perc-title"]} sub-title`}
      >
        LP Total Return %
        <ToolTip
          textStyle={{
            ...
            left: "-0px",
            top: "20px",
            border: props.page === "perpetual" ? "0.5px solid black" : "",
            textAlign: "left",
          }}
          buttonStyle={{ width: 15, height: 15 }}
          text={HelpText[props.page].totalReturnPerc}
        >
          ?
        </ToolTip>
      </div>
      <BacktestTotalReturnPercent
        ...
        loading={props.loading}
        className={`${styles["strategy-breakdown-chart"]} ${
          styles["strategy-breakdown-chart-right"]
        } ${props.pageStyle ? props.pageStyle["inner-glow"] : "inner-glow"}`}
        strategyDropdownClass={styles["strategy-dropdown"]}
        strategy={props.strategy}
        data={props.data}
        amountKey={props.amountKey}
      ></BacktestTotalReturnPercent>
    </div>
  );
};
...
const StrategyBacktest = (props) => {
  ...
  const editableStrategyRanges = useSelector(
    selectSelectedEditableStrategyRanges
  );
  ...
  const [strategyToggled, setStrategyToggled] = useState({
    id: "S1",
    label: "Strategy 1",
  });
  ...
  const rangeValFromPercent = (currentPrice, strategy, key) => {
    return parsePrice(
      parseFloat(currentPrice) +
        parseFloat(currentPrice) *
          parseFloat(strategy.inputs[key].percent / 100)
    );
  };
  ...
  const genChartData = (
    ...
    return strategyRanges.map((d) => {
      const min =
        type === "percent" && d.id !== "v2"
          ? rangeValFromPercent(price, d, "min")
          : d.inputs.min.value;
      const max =
        type === "percent" && d.id !== "v2"
          ? rangeValFromPercent(price, d, "max")
          : d.inputs.max.value;

      const tokenRatio = tokenRatios
        ? tokenRatios.find((t) => d.id === t.id)
        : null;
      const tokens = tokensForStrategy(
        min,
        max,
        investment * d.leverage,
        price,
  ...
  const chartProps = {
    ...
    dataTypeX: "date",
    dataTypeY: "number",
    ylabel: `Daily Fees ${baseToken.symbol}`,
    xlabel: "",
    ...props.chartProps,
  };

  useEffect(() => {
    if (props.customFeeDivisor) {
      setStrategies(editableStrategyRanges);
    } else {
      setStrategies(strategyRanges);
    }
  ...
  useEffect(() => {
    ...
    console.log("fetching hourly pool data for chainid ", protocolID);
    getPoolHourData(
      poolID,
  ...
  useEffect(() => {
    if (
      ...
      pool.token0 &&
      hourlyPoolData &&
      hourlyPoolData.length &&
      pool.id === hourlyPoolData[0].pool.id
    ) {
      const startPrice =
        baseTokenId === 0
          ? hourlyPoolData[0].close
          : 1 / hourlyPoolData[0].close;
      setChartData(
        genChartData(
          strategies,
          pool,
        ...
        if (sR.selected === true) {
          ...
          if (data) {
            const latestChartData = data.data.map((dd, i) => {
              const bar =
                selectedDataTemp && selectedDataTemp[i]
                  ? selectedDataTemp[i]
                  : {};
              bar[sR.id] = dd.feeV;
              bar.date = dd.date;
  ...
  useEffect(() => {
    if (chartData && chartData.length) {
      ...
      const indicators = chartData.map((cD) => {
        return {
          ...cD,
          data: backtestIndicators(
            cD.data,
            investment,
            customCalc,
            strategies.find((d) => d.id === cD.id).hedging
          ),
        };
      });
  ...
  useEffect(() => {
    if (
      ...
      strategies.forEach((s) => {
        const indicator = indicatorsData.find((d) => {
          return d.id === s.id;
        });
  ...
  const handleMouseOver = (group, data, scale, groupScale) => {
    ...
    const text = [
      `Date: ${data.date}`,
      `Daily Fees:`,
      `${parsePrice((hoverData.feeV / investment) * 100, true)} %`,
      `${parsePrice(hoverData.feeV)} ${baseToken.symbol}`,
      !props.customFeeDivisor ? `${parsePrice(hoverData.feeUSD)} USD` : "",
    ];
  ...
  useEffect(() => {
    ...
    if (
      liquidationData &&
      liquidationData.dash &&
      liquidationData.dash.length &&
      entryPrice > 0
    ) {
      liquidationData.dash.forEach((d, i) => {
        const strategy = editableStrategyRanges.find(
          (sR) => sR.color === liquidationData.colors[i]
        );
        if (
          d === true &&
          liquidationData.data[i] &&
          liquidationData.data[i].length > 0 &&
          strategy &&
          strategy.leverage > 1
        ) {
          let l1 = liquidationData.data[i][0].x;
          let l2 =
            liquidationData.data[i][liquidationData.data[i].length - 1].x;

          if (l1 < entryPrice && l2 < entryPrice) {
            liquidationLinesTemp.push({
              y: l2,
              color: liquidationData.colors[i],
            });
          } else {
            liquidationLinesTemp.push({
              y: l1,
  ...
  return (
    <div
      className={`${styles["strategy-backtest-container"]}
      ${props.pageStyle ? props.pageStyle["outer-glow"] : "outer-glow"}
      ${
        props.pageStyle
          ? props.pageStyle["dashboard-section"]
          : "dashboard-section"
      }`}
      onMouseLeave={() => {
        setMouseOverText([]);
    ...
    >
      <div className={`title ${styles["strategy-backtest-title"]}`}>
        ...
        <span
          className={props.pageStyle["help-icon"]}
          style={{ marginLeft: 0 }}
        >
          <ToolTip
            textStyle={{
            ...
            buttonStyle={{ width: 15, height: 15 }}
            text={HelpText[props.page].backtest}
          >
            ?
          </ToolTip>
        </span>
        ...
        <span style={{ fontSize: 12 }}>
          ...
          {entryPrice && entryPrice > 0
            ? `Entry price: ${parsePrice(entryPrice)} ${baseToken.symbol}`
            : ""}
            ...
            return (
              <span style={{ fontSize: 12 }}>
                ...
                {strategyType === "percent"
                  ? d.inputs.min.percent + "%"
                  : d.inputs.min.value + " " + baseToken.symbol}
                &nbsp; Max:{" "}
                {strategyType === "percent"
                  ? d.inputs.max.percent + "%"
                  : d.inputs.max.value + " " + baseToken.symbol}
              </span>
            );
          })}
        </span>
      ...
      <BarChartGrouped
        className={`${
          props.pageStyle ? props.pageStyle["inner-glow"] : "inner-glow"
        } ${styles["strategy-backtest-chart"]}`}
        onMouseLeave={() => {
          setMouseOverText([]);
        }}
        domain={chartDomain}
      ...
      {props.page === "perpetual" ? (
        <BacktestStrategyOverview
          handleLiquidationLines={handleLiquidationLines}
          page={props.page}
          pageStyle={props.pageStyle}
          className={styles["strategy-overview"]}
          currentPrice={
            chartData && chartData[0] ? chartData[0].data[0].baseClose : 0
          }
        ></BacktestStrategyOverview>
      ) : (
        <></>
      ...
      <div className={`${styles["backtest-indicators-container"]}`}>
        <BacktestIndicators
          page={props.page}
          pageStyle={props.pageStyle}
          className={styles["backtest-indicators"]}
          data={selectedIndicatorsData}
          loading={dataLoading}
          supressFields={props.supressIndicatorFields}
        ></BacktestIndicators>
        ...
        {props.page === "perpetual" ? (
          ...
          >
            {liquidationLines.map((d, i) => {
              return d && d.y ? (
                <Line
                  className={styles["liquidation-line"]}
                  useParentScale={true}
                  data={{ y1: d.y, y2: d.y }}
                  stroke={d.color}
                  strokeWidth={1}
        ...
        <StrategyToggle
          ...
          pageStyle={props.pageStyle}
          className={styles["strategy-toggle"]}
          handleToggle={handleStrategyChange}
        ></StrategyToggle>
        <StrategyBreakdown
          loading={dataLoading}
          page={props.page}
          pageStyle={props.pageStyle}
          className={styles["strategy-breakdown-container"]}
          data={chartData}
          strategy={strategyToggled}
          totalReturnKeys={props.totalReturnKeys}
          amountKey={props.amountKey}
        ></StrategyBreakdown>
      </div>
